...
class DumpRecords:
    ...
    def plot_compared_graph(self, config_index_list):
        ...
        # self.plot_assemble_estimation_graph(times, status_vectors, opt_values, figure_dir, "status_opt", "||x-$x^{\star}$||", right=3)
            ...
            opt_value = np.array([[-1, 0.16660663117309651, 0.3333955560508185], [0, -0.8334287020540452, 0.3334118895597235], [1.0001074180788625, 0.16654159249067055, 0.33345503463020215], [0, 1.166571297885446, 0.33341188945528427], [2.0000614803817807, 0.16660663112190888, 3.333395556015464], [-2, 0.16663322749224166, 3.3333734276470937]])
            ...
            if "fixed" in index:
                ...
                initial_value_norms.append(np.linalg.norm(status_vector[0]))
            elif "finite" in index:
                finite_convergence_times.append(convergence_time)
        
        plot_initial_convergence_line__graph(initial_value_norms, [finite_convergence_times, fixed_convergence_times], "$||e_x(0)||$", legneds=["Finite-time algorithmn", "Fixed-time algorithmn"], figure_dir=self.record_root_path+"/compared/"+"@".join(config_index_list)+"/")
