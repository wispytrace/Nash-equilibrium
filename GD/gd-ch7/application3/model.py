...
class Model:
    ...
    def get_target_derivatives(self):
        ...
        # ==========================================
        dot_p_g = np.array([
            2.0 * np.cos(0.2 * t) + 1.0,   # 10 * 0.2 * cos(0.2t) + 1.0
            2.0 * np.cos(0.4 * t),         # 5 * 0.4 * cos(0.4t)
            0.0
        ])
        
        ddot_p_g = np.array([
            -0.4 * np.sin(0.2 * t),        # -2.0 * 0.2 * sin(0.2t)
            -0.8 * np.sin(0.4 * t),        # -2.0 * 0.4 * sin(0.4t)
            0.0
        ])
        ...
        # ==========================================
        radius = 3.0 if not is_uav else 5.0
        group_idx = i % 4
        ...
        if not is_uav:
            ...
            # UAV 原高度: H = 6.0 + 1.5 * sin(0.3 * t)
            dot_h = 0.45 * np.cos(0.3 * t)       # 1.5 * 0.3 * cos(0.3t)
            ddot_h = -0.135 * np.sin(0.3 * t)    # -0.45 * 0.3 * sin(0.3t)
    ...
    def cost_function(self):
        ...
        # 假设目标沿着螺旋线运动
        if is_uav:
            target_pos = np.array([5*np.cos(0.5*self.time), 5*np.sin(0.5*self.time), 2])
        else:
        ...
        for i in range(N):
            if not is_uav:
                status_sum += np.array([self.memory['z'][i,0], self.memory['z'][i,1], 0])
            else:
