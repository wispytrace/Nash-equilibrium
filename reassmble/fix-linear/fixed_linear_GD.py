...
class DumpRecords:
    ...
    def plot_graph(self, record_path=None):
        ...
        ui = self.align_list(memory['update_value'])
        valid_status_vector = []
        valid_speed_vector = []
        valid_acc_vector = []
        ...
        plot_single_status_converge_graph(time, virtual_vector, figure_dir, file_name_prefix="virtual_state", ylabel="$x_{i}$",xlabel_list=["$x_1$", "$x_2$", "$x_3$", "$x_4$"], opt_label_list=["$y_1^*$", "$y_2^*$", "$y_3^*$", "$y_4^*$"])
        ...
        plot_single_status_graph(time, ui, figure_dir, file_name_prefix="ui", ylabel="$u_i$",xlabel_list=["$u_1$", "$u_2$", "$u_3$", "$u_4$"], opt_label_list=None)
        plot_dos_estimate_norm_converge_graph(time, virtual_vector,  estiamte_vector, figure_dir, file_name_prefix="estimate_norm", ylabel="$log_{10}(||z_i$-$x||)$", xlabel_list=["Player 1", "Player 2", "Player 3", "Player 4"], dos_interval=dos_interval)
