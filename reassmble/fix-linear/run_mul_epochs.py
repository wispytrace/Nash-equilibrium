...
class CentralizedModel:
    ...
    def update(self):
        ...
        for i in range(self.num_agents):
            self.agntes[i].update() # 假设 model.update() 返回范数
            ...
            if self.sio:
                try:
                    ...
                    if self.sio.connected:
                        ...
                        NE_vector = np.array([5.749, 15.553, 25.356, 35.160]).reshape(-1, 1)
    ...
    def plot_simulation_result(self, centralized_data=None):
        ...
        # 数据处理
        x_trajs_list = centralized_data["trajectories"]["y"]
        try:
            print(np.array(x_trajs_list).shape)
...
if __name__ == "__main__":
    ...
    for i in range(len(config_list)):
        ...
        magnitudes = [50+i*50 for i in range(TOTAL_SIMULATIONS)]
        ...
        for i, magnitude in enumerate(magnitudes):
            ...
            random_vec = [6.0+ magnitude, 5.0+ magnitude,2.0+ magnitude,7.0+ magnitude] 
