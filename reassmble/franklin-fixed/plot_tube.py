...
def plot_simulation_result(centralized_data=None):
    ...
    for dim in range(1): # 遍历两个维度，分别生成图片
        ...
        for i in range(num_agents):
            plt.plot(time_steps, x_matrix[i, :], color=COLORS[i % len(COLORS)],
                     label=f"$y_{i}$", linewidth=1.2)
        ...
        plt.ylim(-tau_max-0.5, tau_max+6)
    ...
    #     # 保存图片
    #     file_name = f"tube_transient_response_control_dim{dim+1}_sim_{sim_id}.png"
    #     full_path = os.path.join( file_name)
    #     plt.xlim(left=0,right=5)
    #     plt.ylim(top=100)
    #     plt.savefig(full_path, dpi=300)
...
def plot_compare(centralized_data_list, labels):
    NE_vector = np.array([1.61904895, 6.38095369, 7.99999999, 7.99999999 ,7.99999999 , 8.        ])

    for i, centralized_data in enumerate(centralized_data_list):
        time_steps = np.array(centralized_data["time_steps"])
        dist_to_NE = np.zeros_like(time_steps)

        x_matrix = np.array(centralized_data["trajectories"]["x"])
        print(x_matrix.shape)
        # 计算距离
        for j in range(len(dist_to_NE)):
            
            dist_to_NE[j] = np.linalg.norm((x_matrix[:,j,0]-NE_vector).flatten())
...
if __name__ == "__main__":
    with open('/mnt/binghao/NESeeking/Nash-equilibrium/reassmble/franklin-fixed/records/euler_constraint/r_0/sim_101/all_agents_trajectories.json') as f:
