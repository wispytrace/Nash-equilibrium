...
class Model:
    ...
    def status_update_function(self):
        ...
        # --- 修复 3: 使用 solve 代替 inv ---
        # 另外，建议检查 Mi 是否对称正定，如果模拟中出现 Mi 奇异，可以加一个微小的阻尼
        Mi_stable = Mi + np.eye(Mi.shape[0]) * 1e-6 # 正则化，防止奇异
        
        Torque = -Ci @ dot_x - Gi + self.get_ui()
        ddot_x = np.linalg.solve(Mi_stable, Torque)
        
        # --- 修复 4: 增加速度限幅 ---
        # 物理系统不可能有无限速度，如果发散，这里可以截断防止报错
        ddot_x = np.clip(ddot_x, -1e4, 1e4) 
    ...
    def virtual_signal_update_function(self):
        ...
        return 10*(-self.memory['y'] + update_value)
    ...
    def estimation_update_function(self):
        ...
        for i, value in enumerate(self.memory_updation['z']):
            ...
            estimation_update[i] = -1*0.2*(alpha[0]+alpha[1]+alpha[2])*value - 2*self.sign(value)
