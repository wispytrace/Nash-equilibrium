...
class Model:
    ...
    def status_update_function(self):
        """
        状态更新方程，结合新的 M, C, G 计算控制输入并得到加速度 ddot_x
        """
        p = self.model_config.get('p', 1.0)
        q = self.model_config.get('q', 1.0)
        ...
        # 假设 y 为参考轨迹或虚拟信号
        y = self.memory['y'] 
        dot_y = np.zeros_like(dot_x) # 简化：假设参考导数为0
        
        track_error = x - y
        sign_track_error = self.approximate_sign(track_error)
        dot_track_error = np.multiply(dot_x - dot_y, sign_track_error)
        
        # 获取系统矩阵
        Mi, Ci, Gi = self.get_Matrix()
        
        # 控制器设计 (保留原代码结构，但已适配 1x1 矩阵)
        oi = dot_x + h1 * (self.power(track_error, p) + self.power(track_error, q) + track_error)
        self.memory['oi'] = oi
        
        # 计算控制扭矩 tau_i (代码中标记为 ui)
        tau_i = Gi + Ci @ dot_x - h2 * Mi @ (self.power(oi, p) + self.power(oi, q)) \
             - h1 * Mi @ (p * np.multiply(self.power(track_error, p-1), dot_track_error) \
             + q * np.multiply(self.power(track_error, q-1), dot_track_error) + dot_track_error)
             
        self.memory['ui'] = tau_i
        
        # 计算加速度: ddot_qi = Mi^(-1) * (tau_i - Ci*dot_qi - Gi)
        ddot_x = np.linalg.inv(Mi) @ (tau_i - Ci @ dot_x - Gi)
