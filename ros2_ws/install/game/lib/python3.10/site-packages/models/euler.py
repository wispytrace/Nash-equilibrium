...
class Model:
    ...
    def status_update_function(self):
        ...
        self.memory['ui'] = ui
        # self.memory['oi'] = oi
        # self.memory['track_error'] = track_error
        # self.memory['dot_track_error'] = dot_track_error
        # self.memory['doty'] = dot_y
        ddot_x = np.linalg.inv(Mi)@(ui - Ci@dot_x-Gi)
        
        # self.memory['dot_oi_2'] = np.linalg.inv(Mi)@(ui - Ci@dot_x-Gi) + h1*(p*np.multiply(self.power(track_error, p-1),dot_track_error)+ q*np.multiply(
        #     self.power(track_error, q-1), dot_track_error))
        
        
        # self.memory['ui'] = ui
        # if 'oi' in self.memory.keys():
        #     self.memory['dot_oi'] = (oi - self.memory['oi'])/ self.time_delta
        # self.memory['oi'] = oi
        
        # if 'track_error' in self.memory.keys():
        #     self.memory['dot_track_error'] = (h1*(self.power(track_error,p) + self.power(track_error, q)) - self.memory['track_error'])/self.time_delta
        # if 'ddot_track_error' in self.memory.keys():
        #     self.memory['error'] = self.memory['ddot_track_error'] - self.memory['dot_track_error']
        
        # self.memory['track_error'] =  h1*(self.power(track_error,p) + self.power(track_error, q))
        # self.memory['ddot_track_error'] = h1*(p*np.multiply(self.power(track_error, p-1),dot_track_error)+ q*np.multiply(
        #     self.power(track_error, q-1), dot_track_error))
        
        # self.memory['dot_y'] = dot_y
        # self.memory['ddot_X'] = ddot_x
        # self.memory['Gi'] = Gi
        # self.memory['Ci'] = Ci
    ...
    def virtual_signal_update_function(self):
        ...
        for i in range(len(si)):
            for j in range(len(si[i])):
                si[i][j] =  min(np.fabs(self.memory['v'][i][j]), self.model_config['di'][j]) * np.sign(self.memory['v'][i][j])
    ...
    def update(self):
        ...
        norm_value = np.linalg.norm(self.memory['v'],axis=1)
        vi_max = np.max(norm_value)
        ...
        self.memory['gama'] = eta_max[0]*self.power(di, self.model_config['p']) + eta_max[1]*self.power(di, self.model_config['q']) + eta_max[2]*di
