...
class Model:
    ...
    def approximate_sign(self, value, extra = 1e-6):
        # extra = 1e-6
    ...
    def sign2(self, value):
        sign_value = np.zeros(value.shape)
        for i in range(len(value)):
            if np.fabs(value[i]) < 1e-5:
                sign_value[i] = 0
            else:
                sign_value[i] = self.approximate_sign(value[i], extra=1e-2)
    ...
    def status_update_function(self):
        ...
        ui = Gi + Ci@dot_x - h2*Mi@(self.power(oi, p)+self.power(oi, q)) - h1*Mi@(p*np.multiply(self.power(track_error, p-1),dot_track_error)+ q*np.multiply(
            self.power(track_error, q-1), dot_track_error)+ dot_track_error) - 10*self.sign2(oi)
        self.memory['ui'] = ui
    ...
    def project(self):
        ...
        for i in range(len(value_sum)):
            ...
            if value_sum[i] > c:
                # print(c)
                # print(f"start{value_sum[i]}")
                calculate_value = np.copy(values[:,i])
                project_minus[i] = fsolve(self.equation, 0, args=(calculate_value, c , l ,u, ))

                # print(f"end{value_sum[i]}")
            # if value_sum[i] < -c:
            #     lambda_star = fsolve(self.equation, 1, args=(-values[:, i], -c , -l ,-u, ))
            #     for j in range(len(value)):
            #         values[j][i] = values[j][i] + lambda_star
