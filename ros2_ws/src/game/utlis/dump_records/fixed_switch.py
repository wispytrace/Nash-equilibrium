...
class DumpRecords:
    ...
    def plot_status_graph(self, time, status_vector, opt_value, figure_dir, file_name=None, ylabel=None, xlabel='x', ytop=10):
        ...
        for i in range(len(status_vector)):
            y_values = [item[0] if isinstance(item, list) else item for item in status_vector[i]]
            y = np.array(y_values, dtype=float)
            plt.plot(time, y, color=mcolors.TABLEAU_COLORS[colors[i]], label="${}{}(t)$".format(xlabel,self.charater[i+1]))
    ...
    def plot_graph(self):
        ...
        opt_value = [-3.007632442244374, -3.807408455069461, -2.6078307207927387, -2.207915437999935, -1.4074073115710062]
        time = np.array(memory['time'][-1][:len(status_vector[0])])
                
        print(status_vector.shape)
        print(estimate_vector.shape)
        print(time.shape)
        # self.plot_matrix_graph(figure_dir)
        self.plot_status_graph(time, status_vector, status_vector, figure_dir, "Action")
        # self.plot_assembl_estimate_graph(time, status_vector, estimate_vector, opt_value, figure_dir, 0.5)

        # time = time[:len(cost[0])]
        # self.plot_status_graph(time, cost, cost, figure_dir, "cost", "Cost Function",xlabel="f")
