...
class DumpRecords:
    ...
    def plot_status_graph(self, time, status_vector, opt_value, figure_dir, file_name=None, var_name='x', ylabel='Action'):
        ...
        for i in range(shape[0]):
            plt.plot(time, np.array(status_vector[i,:]), color=mcolors.TABLEAU_COLORS[colors[(i)%len(colors)]], label="${}{}$".format(var_name, self.charater[i+1]))
    ...
    def plot_assemble_estimation_graph(self, times, src_vectors, dst_vectors, figure_dir, file_name, y_label='', right=4.5):
        ...
        for i in range(nums):
            ...
            plt.plot(times[i], norms, color=mcolors.TABLEAU_COLORS[colors[i]], label=f"Set {i+1}")
    ...
    def plot_compared_graph(self, config_index_list, figure_dir):
        ...
        self.plot_assemble_estimation_graph(times, virtual_vectors, opt_values, figure_dir, "virtual_status_opt", "||y-$x^{\star}$||" )
        self.plot_assemble_estimation_graph(times, status_vectors, virtual_vectors, figure_dir, "status_virtual", "||x-y||", )
        self.plot_assemble_estimation_graph(times, status_vectors, opt_values, figure_dir, "status_opt", "||x-$x^{\star}$||")
